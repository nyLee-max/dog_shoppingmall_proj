...
<link rel = "stylesheet" href = "<%=request.getContextPath() %>/dog_shopping/css/style.css">
</head>
<body>
	<section id="listForm">
		<c:if test="${dogList != null }">
			<h2>
				개 상품 목록 <a href="dogRegistForm.do">개 상품등록</a>
			</h2>
			<table>
				<tr>
					<c:forEach var="dog" items="${dogList }" varStatus="status">
						<td><a href="dogView.do?id=${dog.id }"> <img
			...
			<table>
				<tr>
					<c:forEach var="todayImage" items="${todayImageList}"
						...
						<c:if test="${((status.index+1) mod 4) == null }">
				</tr>
				<tr>
					</c:if>
					</c:forEach>
